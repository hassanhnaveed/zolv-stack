...
/** The 6 any-to-any document tools + markdown (input-only, fixed pdf output). */
const DOCUMENT_TOOLS: ToolSlug[] = [
  "document-to-pdf",
  "document-to-docx",
  "document-to-odt",
  "document-to-rtf",
...
// Each tool now always outputs a fixed extension (document-to-docx always
// produces .docx no matter the input), so outputExt from TOOL_CONFIG is the
// single source of truth — no special-casing needed anymore.
function toolToTargetFormat(tool: ToolSlug): Exclude<FormatValue, "any"> | null {
  if (!CONVERTER_TOOLS.includes(tool)) return null;
...
// Document formats that flow through the any-to-any tools.
const DOC_PAIR_FORMATS = ["pdf", "docx", "doc", "odt", "rtf", "txt", "html"] as const;

// target format -> which document-to-X tool produces it.
const DOCUMENT_TARGET_TOOL: Partial<
  Record<Exclude<FormatValue, "any">, ToolSlug>
> = {
  pdf: "document-to-pdf",
  docx: "document-to-docx",
  odt: "document-to-odt",
  rtf: "document-to-rtf",
  txt: "document-to-txt",
  html: "document-to-html",
};
...
> = {
  ...
  "avif-pdf": "image-to-pdf",
  "pdf-jpg": "pdf-to-jpg",
  // Dedicated PDF -> Word tool (Python-based) takes priority over the
  // generic document-to-docx LibreOffice route for this specific pair.
  "md-pdf": "md-to-pdf",
};

// Auto-generate every document-format pair (pdf/docx/doc/odt/rtf/txt/html)
// through the any-to-any document tools, without overriding entries already
// set above (e.g. pdf-docx keeps using the dedicated pdf-to-word tool).
for (const source of DOC_PAIR_FORMATS) {
  for (const target of DOC_PAIR_FORMATS) {
    if (source === target) continue;
    const tool = DOCUMENT_TARGET_TOOL[target];
    if (!tool) continue;
    const key = `${source}-${target}` as keyof typeof PAIR_TOOL_MAP;
    if (!(key in PAIR_TOOL_MAP)) {
      PAIR_TOOL_MAP[key] = tool;
    }
  }
}
